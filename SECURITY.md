...
- **`GET /attachments/download/:token` is intentionally unauthenticated** — no bearer required. It exists because some MCP clients reject binary attachments embedded as MCP `resource` content blocks, so `attachments` (`action=download`) instead returns a plain link to this endpoint. The token *is* the credential: 256 bits of randomness (`crypto.randomBytes(32)`), valid for 5 minutes, held only in the server's process memory (`utils/attachment-cache.js`) and never logged. Treat these links like any other bearer-equivalent secret — don't forward them to anyone who shouldn't read the attachment.
